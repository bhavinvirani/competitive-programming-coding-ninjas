...
int main()
{
    ...
    cin >> n >> x >> y;
    long long arr[n];
    long long minimum = INT_MAX, maximum = INT_MIN;
    ...
    long long minimum_cost =  LONG_MAX;
    for (long long i = minimum; i <= maximum; i++)
    {
        long long cost = 0, inc = 0, dec = 0;
        ...
        if(dec <= inc){
            cost = (inc - dec) * y + (dec * x);
            minimum_cost = min(minimum_cost, cost);
        }
        minimum++;
    }
    cout << (minimum_cost != LONG_MAX ? minimum_cost : 0) << endl;
...
//     long long minimum_cost =  INT_MAX;
//     for (long long i = minimum; i <= maximum; i++)
//     {
//         long long cost = 0, inc = 0, dec = 0;
